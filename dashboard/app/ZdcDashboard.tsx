...
async function loadSource(): Promise<LoadedSource> {
  try {
    return { root: "/data", manifest: await fetchJson<Manifest>("/data/manifest.json") };
  } catch {
    return { root: "/demo", manifest: await fetchJson<Manifest>("/demo/manifest.json") };
  }
}
...
export function ZdcDashboard() {
  ...
  useEffect(() => {
    ...
    const request = window.setTimeout(() => {
      setLoading(true);
      fetchJson<EpochArtifact>(`${source.root}/${row.path}`)
        .then((value) => {
          if (!active) return;
    ...
    <main className="dashboard-shell">
      <header className="topbar">
        ...
        <div className="topbar-status">
          ...
          <div>
            <strong>{source.root === "/data" ? "VERTEX SYNCED" : "UI QA FIXTURE"}</strong>
            <small>
              {lastRefresh
      ...
      <section className="analysis-grid analysis-grid--lower">
        ...
        <div className="panel trend-panel">
          <div className="panel-heading">
            ...
            <span className="micro-label">{source.manifest.epochs.length} snapshots</span>
          </div>
