...
\addbibresource{references_v2_1.bib}
\hypersetup{colorlinks=true,linkcolor=blue!55!black,citecolor=blue!55!black,urlcolor=blue!55!black,pdftitle={CBSC-ZDC v2.1 Auditor Specification}}
...
\title{\model\ v2.1: An Audited, CLI-Ready Specification for a Four-Momentum-Conditioned Fast Monte Carlo Surrogate of Neutron Showers in a Zero Degree Calorimeter}
...
\begin{table}[ht]
\centering
\caption{Blocking defects and v2.1 corrections.}
\label{tab:blockers}
...
The active source compiles and all 42 unit tests pass. Branch-aware measured coverage is 51\% over 2,585 statements and 768 branches. Coverage is intentionally reported without inflation: production ROOT I/O, CLI orchestration, evaluator orchestration, and long-running trainer branches remain lightly covered. The tests cover energy convention, zero-kinetic boundary behavior, mass shell, geometry hashing, split contracts, sparse shards, count feasibility, support exactness, both target decoders, removal of latent/reserve variables, profile/event closure, flow-time dependence, causal/bidirectional modes, checkpoint loading, staged trainability, and CLI discovery.
...
\midrule
G1-full & 0--300\,GeV & full v2.1 graph hierarchy & primary broad-support experiment \\
G1-primary & 50--250\,GeV & matched full v2.1 & tests whether outer support helps the primary domain \\
...
\midrule
\endhead
Kinetic/total ambiguity & \texttt{contracts.py}, \texttt{root\_adapter.py} & shell/energy unit tests \\
...
pytest -q
cbsc-zdc inspect-root FILE --schema configs/schema_edm4hep_zdc.yaml
...
cbsc-zdc split --manifest artifacts/data/dataset_manifest.json \
  --output artifacts/splits.json --group-by source_run --seed 20260723
cbsc-zdc audit-dataset --manifest artifacts/data/dataset_manifest.json \
  --splits artifacts/splits.json --split train \
