# Chronological research and design log

## Scope and evidence limitation

The prior 600-item corpus contains mixed evidence depths. Core papers were read at abstract/full-paper level where available; many long-tail entries were screened from titles, abstracts and bibliographic metadata. It would be dishonest to claim a complete line-by-line full-text review of every paper when some papers are paywalled, inaccessible, duplicated by topic, or only metadata-screened. This log preserves the contribution annotation for every item and separates strong evidence from exploratory leads.

## Iteration 1 - physical constraint correction
The proposed monotone layer-deposit rule was rejected because hadronic/neutron showers may start late and grow toward a shower maximum. The constraint was moved to remaining budget using stick breaking.

## Iteration 2 - input contract
The sole raw per-event input was frozen as p4. Geometry is static metadata. Entry coordinates are allowed only as deterministic functions of p4 under a fixed vertex/plane.

## Iteration 3 - longitudinal architecture
A bounded total response, optional start hazard, mixture routing and stick-breaking budget were selected from established techniques. Independent per-stage randomness was rejected in favor of a shared event latent to preserve correlations.

## Iteration 4 - spatial architecture
The default changed from serial previous-layer image generation to parallel all-layer conditional generation. Serial generation remains an ablation because it risks exposure bias, accumulated error and latency.

## Iteration 5 - baseline integration
The supplied single-stage graph-flow proposal was retained as a controlled comparison on identical data, splits and geometry. Its role is diagnostic: determine whether explicit hierarchy earns its complexity. fileciteturn12file0

## Iteration 6 - external ZDC references
The official `m-wojnar/faster_zdc` repository and arXiv:2507.18811 were selected as the highest-priority direct reference. `m-wojnar/zdc` and ExpertSim were added for architecture diversity. Their ALICE outputs and condition vectors differ from the user's 6,790-channel detector, so comparison requires adapters rather than direct reuse.

## Iteration 7 - training range
Training on 0-300 GeV is treated as a hypothesis, not automatically superior. It may stabilize boundaries around 50 and 250 GeV, but it may also allocate capacity to irrelevant regimes. The blueprint therefore includes a matched 0-300 versus 50-250 ablation and keeps primary reporting at 50-250.

## Iteration 8 - no hard gates
Per user direction, the repository contains recommended QA/report points, not mandatory stop/pass gates. Structural invariants remain implementation assertions because violating tensor validity or budget identity is a software error, not a scientific threshold.

## Iteration 9 - repository realization
An executable scaffold was built with a p4 validator, condition encoder, bounded budget cascade, count head, parallel spatial field, exact decoder, diagnostic report, smoke script, configs and tests. It is intentionally not represented as trained or validated.

## Iteration 10 - audit document
The supervisor/auditor write-up distinguishes established components from the proposed combination, lists assumptions and unresolved data-contract issues, and avoids claiming an in-depth full-text review where evidence was only abstract/metadata level.
