...
Production `myTree` clarification established by the 2026-07-24 full scan:

- ECAL readout identity is `ecal_cellID`.
- HCAL readout identity is the composite `(hcal_LayerID, hcal_cellID)`.
- An HCAL readout ID may gang multiple discrete, individually stable physical
  positions from `hcal_posX/Y/Z`.
- The frozen static position for a ganged readout is the unweighted centroid of
  its distinct physical centers. Never weight this centroid by hit frequency,
  energy, event count, split, or target response.
- The geometry artifact records physical-position multiplicity and the ganging
  summary. Unknown IDs or within-center position drift remain hard failures.
