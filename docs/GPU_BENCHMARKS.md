...
**On naming.** The 80 GB datacentre card is referred to by capacity, never by
its model name. `tests/test_qa_policy.py` forbids that model string in every
active-guidance file, because an earlier revision of the policy used access to
that card as a permission screen. The token check is the guard against the
screen coming back; it is not to be relaxed to let a file name the card. Only
immutable July 2026 audits and `logs.md` history still carry the string, as
provenance with no operational force.
...
| 80 GB datacentre GPU | `lr1e4 dicos-p6` | batch 6, 3 trainers on the cluster | 6 | **990.17** | 985.31 | 997.18 |
...
Ratios on equal work: **datacentre / 4090 = 1.524**, **3090 / 4090 = 1.799**,
**3090 / datacentre = 1.180**.
...
The 4090 and datacentre-card epochs were measured while all three pods were training. The
...
### The datacentre card's memory and tensor cores are idle

Peak CUDA memory is **11.74 GB on both the 4090 and the datacentre card** — identical, as
it should be for the same architecture at the same batch size. The datacentre card finishes
with 86.2% of its 80 GB unused; the 4090 uses less than half of its 24 GB.
Half-batch peaks at 5.91 GB.

This explains the ranking. The datacentre card's advantages are capacity, bandwidth and
...
non-tensor-core throughput, where the 4090 is simply the faster part. The datacentre card
...
NPIndex per GPU board-day: **RTX3090 = 79.0**, **80 GB datacentre board = 173.0**, V100 = 70.0,
...
| 80 GB datacentre GPU | 173.0 | **346.00** | 10.71 |
| RTX 4090 | **not listed** | — | — |
...
80 GB datacentre board. Those come from a `readthedocs` mirror marked *Last Updated: Feb. 2022*,
...
| **80 GB datacentre GPU** | 990.17 | **16.50** | 3.636 |
...
| 80 GB datacentre GPU | 7.208 | 14.42 | 0.446 | 346.00 |
...
|---|---:|---:|---:|---:|---:|
| RTX 4090 | — | — | — | — | unpriced |
| 80 GB datacentre GPU | 1.983 | 0.504 | **3.97** | 0.123 | 23.79 |
| RTX 3090 | 1.069 | 0.936 | **2.14** | 0.066 | 12.83 |

**The 3090 is 1.855x more cost-efficient per epoch than the datacentre card** — 2.19x
...
**NTD$13 (USD$0.40)** on the 3090 or **NTD$24 (USD$0.74)** on the datacentre card. Cost is
...
1. **"The 4090 is 3.2x faster than the datacentre card" was wrong.** That rested on an
   datacentre-card rate of 2.30 batch/s sampled while two trainers shared the card. Measured
...
2. **"3090 ahead of the datacentre card is likely but not established" points
   the wrong way.** Measured, the **datacentre card is 1.180x faster than the 3090**. The
   3090's advantage is cost per epoch, not speed.

3. **The NT$ price figures were 2.5x too high** — see above.

4. **"Running two families in parallel across the 4090 and the datacentre card is slower
...
- Anything about `amp`, larger batches, or larger models — every figure here is
  ...
  datacentre card would be expected to close or reverse the gap.
- Solo epoch times for the 4090 and the datacentre card; theirs were measured under ~2%
  cluster contention.
