...
| `v3bat3-v3-m0-fresh-e19` | RTX 3090 | schema-v3 M0 fixed-bank battery; S1/S2/S3 queue automatically | ~80 min per eligible checkpoint |
...
```bash
MSYS_NO_PATHCONV=1 DICOS_CONFIG="C:/Users/Julia/.dicos/config_3090.json" python scripts/dicos.py logs v3bat3-v3-m0-fresh-e19
```
...
**The battery.** Valid schema-v3 reports under `_v3/battery/` record
...
downstream reconstruction metric. They explicitly distinguish validation truth,
generated, memorization-only training-reference, and test-event counts. B0 now
has one accepted schema-v3 report over 10,000 validation pairs; its high-level
C2ST remains poor at 0.774766, so physics validation is not established. B0's
first report is quarantined because zero-truth events inflated
...
unstable. The next container is quarantined because it understated the 2,000-
event memorization training reference as zero; its metric values were migrated
unchanged into schema v3. The old S1 wrapper stop did
