...
class GeometryAwareNodeField(nn.Module):
    def __init__(self,node_dim:int,edge_dim:int,cond_dim:int,n_layers:int,hidden:int=96,blocks:int=3,heads:int=4,context_layers:int=2,mode:str='bidirectional',state_dim:int=0,axis_dim:int=0):
        super().__init__(); self.n_layers=n_layers; self.state_dim=state_dim; self.axis_dim=int(axis_dim)
        self.time=SinusoidalTimeEmbedding(32) if state_dim else None
        # The four incident-axis columns are appended after the existing static
        # node features. A migrated v2 checkpoint zero-initializes exactly this
        # block, so the projection reproduces its parent before fine-tuning.
        input_dim=node_dim+self.axis_dim+cond_dim+2+state_dim+(32 if state_dim else 0)
    ...
    def forward(self,node_features,edge_index,edge_features,layer_index,cond,layer_energy,count_fraction,state=None,t=None,support_mask=None,axis=None):
        b=cond.shape[0]; n=node_features.shape[0]
        pieces=[node_features[None].expand(b,-1,-1)]
        if self.axis_dim:
            if axis is None:
                raise ValueError('axis features are declared but were not supplied')
            if axis.shape!=(b,n,self.axis_dim):
                raise ValueError(f'axis features must have shape {(b,n,self.axis_dim)}, got {tuple(axis.shape)}')
            pieces.append(axis)
        elif axis is not None:
            raise ValueError('axis features were supplied to a field that does not declare them')
        pieces+=[cond[:,None].expand(-1,n,-1),layer_energy[:,layer_index,None],count_fraction[:,layer_index,None]]
        if self.state_dim:
